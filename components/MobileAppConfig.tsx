...
import { UserRole, Project, MobileModuleConfig } from '../types';

interface MobileAppConfigProps {
  project: Project;
  onUpdate: (config: MobileModuleConfig) => void;
  role: UserRole;
}

const MobileAppConfig: React.FC<MobileAppConfigProps> = ({ project, onUpdate, role }) => {
  const isViewOnly = role === UserRole.PROJECT_MANAGER;

  const config: MobileModuleConfig = project.mobileConfig || {
    appleAppId: '',
    googlePlayId: '',
    bundleId: '',
    features: [
  ...
  const handleUpdateField = (field: keyof MobileModuleConfig, value: string) => {
              ...
              <div key={f.name} className="flex items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100">
                 ...
                 <select 
                    ...
                    onChange={(e) => {
                      ...
                      newFeatures[idx].status = e.target.value;
                      onUpdate({ ...config, features: newFeatures });
                    }}
                    className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border-none outline-none ${f.color}`}
                 >
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  value={config.appleAppId}
                  onChange={(e) => handleUpdateField('appleAppId', e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-1">
                ...
                <input 
                  ...
                  value={config.googlePlayId}
                  onChange={(e) => handleUpdateField('googlePlayId', e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-1">
                ...
                <input 
                  ...
                  onChange={(e) => handleUpdateField('bundleId', e.target.value)}
                />
              </div>
           </div>
        </div>
